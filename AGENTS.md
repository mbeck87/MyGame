...
├── systems/             # audio, weapons, effects, services
├── ui/                  # menu UI
...
For looped sounds, use `audio.start_loop(...)`, `audio.update_loop(...)`, and `audio.stop_loop(...)`. User-controlled master volume is available through the ESC options slider and persists in `settings.json`.
...
Controls: WASD moves/drives, mouse aims, LMB/SPACE shoots, E enters/exits cars, F robs pedestrians, 1-6 switch weapons, R restarts after game over, and ESC opens quit/options flow.
